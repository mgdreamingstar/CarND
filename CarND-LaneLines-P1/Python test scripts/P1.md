...
mask = np.zeros_like(edges) # make *edge* to black
ignore_mask_color = 255

# This time we are defining a four sided polygon to mask
imshape = image.shape
vertices = np.array([[(0,imshape[0]),(450, 300), (500, 300), (imshape[1],imshape[0])]], dtype=np.int32)
cv2.fillPoly(mask, vertices, ignore_mask_color) # make *other* exclude *edge* to white
masked_edges = cv2.bitwise_and(edges, mask) # reverse color = white to black & black to white
...
1. slope Add and average: (y2-y1)/(x2-x1)
2. compute left x and right x (on both start and end)
    1. left x


    right_y_start = img.shape[0]
    right_x_start = (right_y_start - right_line[1]) / right_line[0]
    right_x_end = min(right_x)
    right_y_end = right_line[0] * right_x_end + right_line[1]
